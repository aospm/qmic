				...
				char *indent, char *parent)
{
	...
	printf("struct %s, indent %ld\n", qs->name, strlen(indent));

	if (indent[0] != '\0')
		fprintf(fp, "%sstruct %s%s_%s {\n", indent, package, parent, qs->name);
	else
	...
	list_for_each_entry(qsm, &qs->members, node) {
		...
		if (qsm->struct_ch) {
			...
			if (strlen(parent) + strlen(qs->name) > QMI_STRUCT_TYPE_NAME_MAX) {
				printf("autogenerated struct type %s_%s longer than %d", parent, qs->name, QMI_STRUCT_TYPE_NAME_MAX);
				exit(1);
			}
			strcpy(parent + strlen(parent), "_");
			strcpy(parent + strlen(parent), qs->name);
